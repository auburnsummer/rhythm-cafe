...
import { LoadingState, API_URL, groupBy, uniqBy } from "../utils";
        ...
        .then(({data}) => {
            ...
            return Object.entries(groups).map(([key, rows]) => {
                const tags = rows[0].tag == null ?
                    [] : 
                    uniqBy(x => x.tag_seq, rows).sort((a, b) => a.tag_seq - b.tag_seq).map(x => x.tag);
                const authors = uniqBy(x => x.author_seq, rows).sort((a, b) => a.author_seq - b.author_seq).map(x => x.author);
                return {
                    ...rows[0],
                    tags,
                    authors
